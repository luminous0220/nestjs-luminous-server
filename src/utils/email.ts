...
export const sendEmail = async (
  ...
  const transport = nodemailer.createTransport({
    host: process.env.SMTP_HOST, // qq邮箱主机 host
    secure: true, // 如果为true，则连接将在连接到服务器时使用 TLS。如果为false（默认值），则如果服务器支持STARTTLS扩展，则使用 TLS。在大多数情况下，如果连接到端口465，请将此值设置为true
    port: 465, // SMTP 端口
    auth: {
      user: process.env.SMTP_USER, // 账号：域名邮箱账号
      pass: process.env.SMTP_PASS // 密码：SMPT获取的授权密码
    }
  })
  try {
    let htmlStr = ''
    ...
    const info = await transport.sendMail({
      // 发送者的信息
      from: process.env.SMTP_USER, // 也可以是 "sendname <sender@server.com>""
      // 接受者的信息
      to,
      // 主题
